...
class CustomLineChartUserWidget extends StatefulWidget {
  ...
  @override
  State<CustomLineChartUserWidget> createState() => _CustomLineChartUserWidgetState();
}

class _CustomLineChartUserWidgetState extends State<CustomLineChartUserWidget> {
  ...
  List<Color> userTimeSpentStudying = [
    Colors.redAccent,
    Colors.greenAccent,
  ];
  List<Color> userTimeSpentOnExams = [
    Colors.blueAccent,
    Colors.blueGrey,
  ];
  double _maxY = 0;
  ...
  Map<int, String> _calculateLeftTitles(List<CustomLineChartDataPoint> data) {
    ...
    for (int i = 0; i <= range; i += interval.ceil()) {
      int value = int.parse((_minY + i).toString());
  ...
  void initState() {
    ...
    metricTypeData = {
      ...
      'Courses Completed over Time': {
        'data': sampleUserExamsCompletedOverTimeData.map((data) => FlSpot(data.x, data.y)).toList(),
        'colors': coursesCompletedGradientColors,
      },
      'User Activity': {
        'data': sampleUserActivity.map((data) => FlSpot(data.x, data.y)).toList(),
        'colors': userActivityGradientColors,
      },
      'Courses Enrolled over Time': {
        'data': sampleUserCoursesEnrolledOverTimeData.map((data) => FlSpot(data.x, data.y)).toList(),
        'colors': coursesEnrolledGradientColors,
      },
      'Time Spent on Studying': {
        'data': sampleUserTimeSpentStudying.map((data) => FlSpot(data.x, data.y)).toList(),
        'colors': userTimeSpentStudying,
      },
      'Time Spent on Exams': {
        'data': sampleUserTimeSpentOnExams.map((data) => FlSpot(data.x, data.y)).toList(),
        'colors': userTimeSpentOnExams,
      },
    };
  }
  ...
  Widget build(BuildContext context) {
    Map<String, List<Color>> dataTypesAndColors = {
      ...
      'Time Spent on Studying': userTimeSpentStudying,
      'Time Spent on Exams': userTimeSpentOnExams,
  ...
  Widget leftTitleWidgets(double value, TitleMeta meta) {
    var titles = _calculateLeftTitles(_selectedMetricType == 'All'
        ? [
            ...sampleUserExamsCompletedOverTimeData,
            ...sampleUserActivity,
            ...sampleUserCoursesEnrolledOverTimeData,
            ...sampleUserTimeSpentStudying,
            ...sampleUserTimeSpentOnExams,
          ]
